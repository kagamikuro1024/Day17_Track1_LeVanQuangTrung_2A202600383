...
2. In-Scope có 3 tính năng — OK (không quá 5)

3. Non-Goals có nhiều thứ team muốn làm: hybrid deployment, ReAct agent, TA dashboard — OK
...
- Lý do chọn:
  - Cost: ~$0.01-0.05/query → với 240 students × 3 queries/week = ~$150-750/tháng → chấp nhận được
  - Accuracy: State-of-the-art cho programming questions, instruction-following tốt
  - Latency: ~1-2s API call → tổng response time ≤5 phút (bao gồm retrieval)
...
- Trigger:
  ...
  - HOÀC response chứa pattern "I'm not sure", "I don't have enough information"
  - HOÀC retrieval hit rate < 50%
- Hành động:
...
Tôi sẽ chạy AI Stress-Test prompt sau đây với toàn bộ PRD:
...
Is it truly actionable, or is a vanity metric in disguise?
...
Tôi sẽ chạy prompt này sau khi paste toàn bộ nội dung từ MVP Boundary + PRD + Hypothesis vào, và sau đó cập nhật **Action: Accept / Reject / Partial** với lý do.

*(Tôi sẽ chạy AI critique sau khi hoàn thiện bản này và cập nhật vào đây)*

---

**Thay đổi lớn nhất giữa Version A và Version B:**

*(Sẽ cập nhật sau khi chạy AI critique)*
...
**Next steps:**
1. Chạy AI Stress-Test với prompt trên
2. Chỉnh sửa theo feedback
3. Nộp bản A trên LMS trước 13:00
4. Sau đó optimize bản B cho submission GitHub
